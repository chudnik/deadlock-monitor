...
public:
     ...
     * @return @code true@endcode Если запрос был успешно удовлетворен (сразу или после ожидания).
     * @return @code false@endcode Если монитор был остановлен (shutdown) во время ожидания потока.
     ...
     * @return Объект @code StateSnapshot@endcode с актуальными векторами выделения и потребностей.
     */
    StateSnapshot snapshot() const;

    /**
     * @brief Получение накопленной статистики по всем потокам.
     * @return Вектор структур @code ThreadStats@endcode, где индекс соответствует идентификатору потока.
     */
...
private:
     ...
     * @return @code true@endcode Если состояние безопасное (существует последовательность завершения всех потоков).
     * @return @code false@endcode Если состояние небезопасное (есть угроза взаимной блокировки).
     */
    bool isSafe() const;
